...
- **Status**: Accepted
- **Context**: The user requested that corporate PMO registration should not require entering the Company Name manually; only the Company Registration Number (Company ID) should be entered.
- **Decision**: Remove the "Company Name" input field from Step 2 of the registration page. Make the "Company Registration No." field full-width and rename/label it as "Company Registration No. (Company ID)" for better clarity. In Step 3, automatically resolve and show the mock company name based on the entered Company ID using a localized lookup registry in JavaScript.
- **Consequences**: Streamlines the corporate onboarding flow by eliminating redundant user entry while maintaining verification visual cues (mock-resolving the company name on Step 3).
