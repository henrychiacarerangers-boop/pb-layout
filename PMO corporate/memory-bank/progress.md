...
- **Simplified Corporate Registration**: Removed manual Company Name input in `register.html` (Step 2) in favor of entering only the Company Registration No. (Company ID). Integrated automatic resolution of company names via lookup registry in `js/register.js` to showcase them dynamically on Step 3.
